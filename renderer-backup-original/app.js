...
const api = T
  ? {
      ...
      stateExport: () => T.core.invoke('state_export'),
      stateImport: (mode) => T.core.invoke('state_import', { mode: mode ?? 'merge' }),
    }
  : window.vll
...
function renderSidebar() {
  ...
  if (!models.length) {
    ...
    // front costs more than anyone can scroll. Render the first batch and
    // offer a button to load the rest, keeping the UI responsive while still
    // letting a user reach the long tail without guessing search terms.
    const batch = state.sidebarShowAll ? models.length : 60
    models.slice(0, batch).forEach((m) => list.appendChild(chipEl(m)))
    if (models.length > batch) {
      const remaining = models.length - batch
      list.insertAdjacentHTML(
        'beforeend',
        `<button class="load-more" type="button" data-more="sidebar">Show ${remaining} more</button>`
      )
    }
  }
...
function renderLanes() {
  ...
  // If the user is editing a lane name, don't rebuild that lane: replacing the
  // contenteditable would steal focus and caret position mid- keystroke. Other
  // lanes and the rest of the UI still update underneath.
  const active = document.activeElement
  const editingName = active?.classList?.contains('lane-name') ? active : null
  const editingSlug = editingName?.closest('.lane')?.dataset.lane ?? null
  let editingLane = null
  if (editingSlug) {
    editingLane = state.lanes.find((l) => l.slug === editingSlug)
    if (!editingLane) editingName = null
  }

  // Preserve the editing lane's current element so its input state survives.
  let keptEditor = null
  if (editingName) {
    const oldLaneEl = editingName.closest('.lane')
    if (oldLaneEl) {
      keptEditor = oldLaneEl
      oldLaneEl.remove()
    }
  }

  // Remove everything else before rebuilding so we don't double lanes if the
  // order changed.
  host.querySelectorAll('.lane').forEach((el) => el.remove())
  ...
  state.lanes.forEach((lane) => {
    if (lane.slug === editingSlug && keptEditor) {
      host.appendChild(keptEditor)
    } else {
...
const notif = {
  ...
  /** Mute a kind everywhere: `kind`. */
  mutedKinds: new Set(JSON.parse(receipts.getItem('notif.mutedKinds') || '[]')),
  /** Mute a kind in one lane: `lane:kind`. */
  mutedLaneKinds: new Set(JSON.parse(receipts.getItem('notif.mutedLaneKinds') || '[]')),
}
...
function notifPersist() {
  ...
  receipts.setItem('notif.mutedKinds', JSON.stringify([...notif.mutedKinds]))
  receipts.setItem('notif.mutedLaneKinds', JSON.stringify([...notif.mutedLaneKinds]))
}

/** True if this incident is muted, either globally by kind or in its lane. */
function isMuted(incident) {
  return (
    notif.mutedKinds.has(incident.kind) || notif.mutedLaneKinds.has(`${incident.lane}:${incident.kind}`)
  )
}
...
const unreadIncidents = () =>
  windowedIncidents().filter(
    (i) => !SILENT_KINDS.has(i.kind) && !notif.read.has(incidentKey(i)) && !isMuted(i)
  )
...
function renderNotifCenter() {
  ...
  const laneOptions = state.lanes
    .map((l) => `<option value="${attr(l.slug)}"${l.slug === notifLaneFilter ? ' selected' : ''}>${attr(l.name)}</option>`)
    .join('')
  const scopeBar = `
    <div class="notif-scope">
      <label class="notif-scope-label">Lane</label>
      <select class="notif-scope-select" id="notifLaneSelect">
        <option value=""${!notifLaneFilter ? ' selected' : ''}>All lanes</option>
        ${laneOptions}
      </select>
      ${notifLaneFilter ? '<button class="notif-scope-clear" type="button">Show all lanes</button>' : ''}
    </div>`

  if (!groups.length) {
    list.innerHTML = scopeBar + `<div class="empty-state"><strong>Nothing to report</strong>${
      notifLaneFilter ? 'No failures on this lane in the last 24 hours.' : 'No failures recorded in the last 24 hours.'
    }</div>`
    return
  }

  list.innerHTML = scopeBar + groups
    .map(({ count, latest }) => {
      const d = DIAGNOSIS[latest.kind] || DIAGNOSIS.unattributed
      const lane = state.lanes.find((l) => l.slug === latest.lane)
      const mutedGlobal = notif.mutedKinds.has(latest.kind)
      const mutedLane = notif.mutedLaneKinds.has(`${latest.lane}:${latest.kind}`)
      const muted = mutedGlobal || mutedLane
      const fix = !muted && lane && d.fix && d.fix(latest, lane)
      const laneMuteLabel = mutedLane
        ? `Ignored in ${attr(latest.lane)} — restore`
        : `Ignore in ${attr(latest.lane)}`
      return `
      <article class="issue${muted ? ' is-muted' : ''}">
        ...
        <div class="issue-actions">
          ...
          <button class="btn-ghost issue-mute" data-mute="${attr(latest.kind)}" title="Silence this type everywhere">${
            mutedGlobal ? 'Ignored everywhere — restore' : 'Ignore everywhere'
          }</button>
          <button class="btn-ghost issue-mute-lane" data-lane="${attr(latest.lane)}" data-kind="${attr(latest.kind)}" title="Silence only in this lane">${laneMuteLabel}</button>
        </div>
      </article>`
    })
    .join('')
...
$('notifScrim').addEventListener('click', (event) => {
  ...
  if (mute) {
    ...
    if (notif.mutedKinds.has(kind)) notif.mutedKinds.delete(kind)
    else notif.mutedKinds.add(kind)
    notifPersist()
    renderNotifCenter()
    renderBell()
    return
  ...
  // Per-lane mute is narrower: the same kind in another lane still announces.
  const muteLane = event.target.closest('.issue-mute-lane')
  if (muteLane) {
    const key = `${muteLane.dataset.lane}:${muteLane.dataset.kind}`
    if (notif.mutedLaneKinds.has(key)) notif.mutedLaneKinds.delete(key)
    else notif.mutedLaneKinds.add(key)
    notifPersist()
    renderNotifCenter()
    renderBell()
    return
...
// Lane filter dropdown inside the notification center. Delegated on the scrim
// because the list (and therefore the select) is re-rendered in place.
$('notifScrim').addEventListener('change', (event) => {
  const laneSelect = event.target.closest('#notifLaneSelect')
  if (laneSelect) {
    notifLaneFilter = laneSelect.value || null
    renderNotifCenter()
  }
})
...
// Load-more buttons in sidebar and browse replace the old hard caps.
document.addEventListener('click', (event) => {
  const more = event.target.closest('[data-more]')
  if (!more) return
  if (more.dataset.more === 'sidebar') {
    state.sidebarShowAll = true
    renderSidebar()
  } else if (more.dataset.more === 'browse') {
    state.browseShowAll = true
    renderBrowse()
  }
})
...
async function loadCatalog() {
  ...
  try {
    ...
    state.catalogStale = result.stale || false
    state.catalogRetainedAt = result.retained_at || 0
    ...
    const staleSignature = `${state.catalogStale ? 'stale' : 'fresh'}:${state.catalogErrors.map((e) => e.provider_id).sort().join(',')}`
    if ((state.catalogErrors.length || state.catalogStale) && staleSignature !== loadCatalog._notifiedFor) {
      loadCatalog._notifiedFor = staleSignature
      state.catalogErrors.forEach((e) => {
        toast(`${e.provider_name}: catalog failed — using the last good cache`, String(e.error))
      })
      if (state.catalogStale && !state.catalogErrors.length) {
        const when = state.catalogRetainedAt
          ? new Date(state.catalogRetainedAt * 1000).toLocaleString()
          : 'earlier'
        toast('Serving from stale catalog cache', `Last good catalog was retained ${when}. Add or refresh a provider to update it.`)
      }
    } else if (!state.catalogErrors.length && !state.catalogStale) {
...
$('pExport').addEventListener('click', async () => {
  try {
    const path = await api.stateExport()
    toast('Setup exported', path)
  } catch (err) {
    const msg = String(err)
    if (!msg.includes('cancelled')) note(msg, 'bad')
  }
})
...
$('pImport').addEventListener('click', async () => {
  ...
  try {
    const path = await api.stateImport(mode)
    await loadProviders()
    await loadLanes()
    await loadPool()
    loadCatalog()
    render()
    toast(`Setup imported (${mode})`, path)
  } catch (err) {
    const msg = String(err)
    if (!msg.includes('cancelled')) note(msg, 'bad')
  }
...
function renderBrowse() {
  ...
  // costs more than anyone will scroll through, so render the first batch and
  // offer a button to load the rest.
  const batch = state.browseShowAll ? models.length : 80
  models.slice(0, batch).forEach((m) => list.appendChild(rowEl(m, cols)))
  if (models.length > batch) {
    const remaining = models.length - batch
    list.insertAdjacentHTML('beforeend',
      `<button class="load-more" type="button" data-more="browse">Show ${remaining} more</button>`)
  }
}
...
$('bSearch').addEventListener('input', (e) => { state.browse.search = e.target.value; state.browseShowAll = false; renderBrowse() })
$('bAuthor').addEventListener('change', (e) => { state.browse.author = e.target.value; state.browseShowAll = false; renderBrowse() })
$('bProvider').addEventListener('change', (e) => { state.browse.provider = e.target.value; state.browseShowAll = false; renderBrowse() })
$('bContext').addEventListener('change', (e) => { state.browse.context = Number(e.target.value); state.browseShowAll = false; renderBrowse() })
$('bPrice').addEventListener('change', (e) => { state.browse.price = e.target.value; state.browseShowAll = false; renderBrowse() })
